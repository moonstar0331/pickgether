...
public interface VoteCommentRepository extends JpaRepository<VoteComment, Long> {
    ...
    @Query("SELECT vc FROM VoteComment vc LEFT JOIN CommentLike cl ON vc.id = cl.voteComment.id GROUP BY vc.id ORDER BY COUNT(cl.id) DESC")
    List<VoteComment> findAllOrderByCommentLikeCountDesc();
}
